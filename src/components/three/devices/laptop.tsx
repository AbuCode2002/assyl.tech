...
export function Laptop({ screen, lidRef, ref }: { screen: THREE.ShaderMaterial; lidRef: Ref<THREE.Group>; ref?: Ref<THREE.Group> }) {
  ...
  const parts = useMemo(() => {
    ...
    const hinge = new THREE.CylinderGeometry(0.035, 0.035, LAPTOP.w - 0.6, 24);
    hinge.rotateZ(Math.PI / 2);
    const logoPlane = new THREE.PlaneGeometry(0.62, 0.42);

    const alu = new THREE.MeshPhysicalMaterial({ color: "#23262e", metalness: 0.95, roughness: 0.34, clearcoat: 0.25 });
    const bezelMat = new THREE.MeshPhysicalMaterial({ color: "#030305", metalness: 0.2, roughness: 0.08, clearcoat: 1 });
    ...
    const padMat = new THREE.MeshPhysicalMaterial({ color: "#1b1e25", metalness: 0.7, roughness: 0.22, clearcoat: 0.6 });
